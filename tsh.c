...
void eval(char *cmdline) {
    ...
    if (strcmp(argv[0], "quit") == 0 || strcmp(argv[0], "jobs") == 0 || strcmp(argv[0], "bg") == 0 || (strcmp(argv[0], "fg") == 0))
    {
        ...
        if (r > 0) {
            ...
            setpgid(0, 0); 
            //We have two cases
            // 1: If the process we are going to run is in the background
            // 2: If the process is in the foreground
            int isBackground = (*argv[argc-1] == '&');
            if(isBackground){ 
                // Case 1: The background Process 
                // Disconnect from stdio (meaning close file desc?)
                close(STDIN_FILENO);
                close(STDOUT_FILENO);
                // Get rid of & 
                argv[argc-1] = '\0'; 
                ...
                //What happens now? The program is done executiing wtf do we do? 

            }   
            else{
                // Case 2: The foreground process
                error = execv(argv[0],argv);
